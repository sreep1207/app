...
spec:
  ...
  - name: kaniko
    ...
    command: ["sh", "-c", "/kaniko/executor --dockerfile=/var/jenkins_home/workspace/app/Dockerfile --context=/var/jenkins_home/workspace/app --destination=sree1207/my-app15:${RELEASE}-${env.GIT_COMMIT} && sleep infinity"]
    volumeMounts:
      - name: kaniko-secret
